...
def calculate_severity(img_bytes):
    ...
    if total_leaf == 0: return 0.0, "Healthy"
    severity_percentage = (cv2.countNonZero(infected_mask) / total_leaf) * 100
    if severity_percentage < 5: stage = "Healthy / Early Stage"
    elif severity_percentage < 25: stage = "Mild Infection"
    elif severity_percentage < 50: stage = "Moderate Infection"
    else: stage = "Severe Infection (Action Required Immediately!)"
...
if uploaded_file is not None:
    ...
    if st.button(f"🔊 {labels['listen']}"):
        audio_text = f"{info['disease_name']}. {labels['symptoms']}: {info['symptoms']}. {labels['chemical']}: {info['chemical_treatment']}"
